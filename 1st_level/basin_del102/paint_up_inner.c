										 ...
										 unsigned char* re_fdir, unsigned short* basin, float* dem) 
{
	...
	for (uint i = 0;i < inner_num; i++) {
		...
		color = 10000 + i;
		paint_rc(re_fdir, basin, &rim_cell[i], idx, color, div, offset);
...
int paint_rc(unsigned char* re_fdir, unsigned short* basin, u64_List* rim_cell_i, 
	         unsigned long long idx, unsigned short color, 
			 const unsigned char div[], const int offset[]) {

	// paint and get rim_cell
	basin[idx] = color;
	uint8 reverse_fdir = re_fdir[idx];

	if (reverse_fdir == 0) {
		u64_List_append(rim_cell_i, idx, L_SIZE);
	}
	else {
		for (int i = 0; i < 8; i++) {
			if (reverse_fdir >= div[i]) {
				paint_rc(re_fdir, basin, rim_cell_i, idx + offset[i], color, div, offset);
				reverse_fdir -= div[i];
			}
		}
	}
