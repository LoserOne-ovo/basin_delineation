...
def create_args():
    ...
    :return:
    """
    parser = argparse.ArgumentParser(description="Set part of .tif image to be nodata via a line .shp!",
                                     epilog="Projections of the demarcation line should not have overlapped parts "
                                            "in horizontal when method in [1,2]; and not have overlapped parts "
                                            "in vertical when method in [3,4].")
    parser.add_argument("line_shp", help="demarcation line .shp")
    parser.add_argument("dir_tif", help="input dir .tif")
    parser.add_argument("method", help="1 means only part of the GeoTIFF file that above the input line will "
                                       "be reserved, the rest part will be set nodata;"
...
def mask_above_or_below(line_shp_fn, dir_tif_fn, method=1):
    """
    Set the above(or below) part of the image to no-data
    :param line_shp_fn:
    :param dir_tif_fn:
    :param method: 1 means below; 2 means above
    :return:
    """
    # 读取栅格数据和辅助线数据
    ...
    if method == 1:
        for i in range(cols):
            dir_arr[bound_line[i]:, i] = raster.dir_nodata
    else:
        for i in range(cols):
            dir_arr[0:bound_line[i], i] = raster.dir_nodata
    ...
    raster.array2tif(out_fn, dir_arr, geo_trans, proj, nd_value=raster.dir_nodata, dtype=raster.OType.Byte)
...
def mask_left_or_right(line_shp_fn, dir_tif_fn, method=3):
    """
    Set the left(or right) part of the image to no-data
    :param line_shp_fn:
    :param dir_tif_fn:
    :param method: 3 means right; 4 means left
    :return:
    """
    # 读取栅格数据和辅助线数据
    dir_arr, geo_trans, proj = raster.read_single_tif(dir_tif_fn)
    ...
    if method == 3:
        ...
        for i in range(rows):
            dir_arr[i, 0:bound_line[i]] = raster.dir_nodata
...
def get_points(line_shp_fn):
    """
    Get points of the demarcation line
    :param line_shp_fn:
    :return:
    """
...
def check_in_envelope(x, y, envelope):
    """
    check whether the point locates in the boundary of the image
    :param x:
    :param y:
    :param envelope:
    :return:
    """
    if envelope[0] < x < envelope[1] and envelope[3] < y < envelope[2]:
        return True
    else:
        return False


def main(line_shp, dir_tif, method=1):
    """
    ...
    # 具体的业务函数
    if method < 3:
        mask_above_or_below(line_shp, dir_tif, method)
    else:
        mask_left_or_right(line_shp, dir_tif, method)


if __name__ == "__main__":

    args = create_args()
    main(args.line_shp, args.dir_tif, args.method)
